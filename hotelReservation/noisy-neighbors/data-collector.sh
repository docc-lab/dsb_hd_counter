...
main() {
    if [[ $# -eq 0 ]]; then
        ...
        echo ""
        echo "# Perf instrumentation (per-request via timing interceptor):"
        ...
        echo "#PERF_EVENTS=''             # Empty to disable perf (timing only)"
        echo ""
        echo "# Jaeger tracing (optional):"
        echo "JAEGER_SAMPLE_RATIO=0.01    # 1% sampling, 0 to disable"
        echo ""
        echo "# wrk2 workload (optional):"
