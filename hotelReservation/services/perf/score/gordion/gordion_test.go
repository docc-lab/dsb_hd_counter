...
func TestDespikerRejectsImpulses(t *testing.T) {
	// The observed environmental pattern: a healthy ~100 kcyc stream with
	// 300x impulses every 11th window. Every impulse must be replaced by
	// a value near the healthy level.
	d := NewDespiker(30, 8)
	for i := 0; i < 200; i++ {
		v := 100.0
		if i > 10 && i%11 == 0 {
		...
		got := d.Push(v)
		if got > 200 {
			t.Fatalf("window %d: impulse leaked through despiker: %v", i, got)
		}
	}
}

func TestDespikerPassesModerateShift(t *testing.T) {
	// Real contention (< k x baseline) must pass through untouched from
	// the first shifted window.
	d := NewDespiker(30, 8)
	for i := 0; i < 40; i++ {
		d.Push(100.0)
	}
	if got := d.Push(500.0); got != 500.0 {
		t.Fatalf("5x sustained shift must not be clipped (k=8), got %v", got)
	}
}

func TestDespikerAdaptsToExtremeSustainedShift(t *testing.T) {
	// A sustained shift ABOVE k x median is clipped at first but must
	// pass through once the raw-history median catches up (~window/2).
	d := NewDespiker(30, 8)
	for i := 0; i < 40; i++ {
		d.Push(100.0)
	}
	passed := -1
	for i := 0; i < 30; i++ {
		if got := d.Push(2000.0); got == 2000.0 {
			passed = i
			break
		}
	}
	if passed < 0 {
		t.Fatal("sustained 20x shift never passed the despiker; median must track raw history")
	}
	if passed > 20 {
		t.Fatalf("sustained shift took %d windows to pass (want <= ~window/2)", passed)
	}
}

func TestScorerDespikeKeepsBaselineLow(t *testing.T) {
	// End-to-end: metronome-contaminated baseline stream. Without
	// despiking the Gaussian smoother pins y50; with despike_k=8 the
	// score must stay near zero.
	cfg := testConfig()
	cfg.DespikeK = 8
	sc := NewScorer(cfg, testCurve(t))
	var out Output
	for i := 0; i < 300; i++ {
		w := baselineWindow()
		if i > 10 && i%11 == 0 {
			w.P50Ns, w.P90Ns = 25_000_000, 40_000_000 // 25/40 ms stall window
		}
		out = sc.Score(w)
	}
	if !out.OK {
		t.Fatal("expected scored output")
	}
	if out.Y50 > 0.2 || out.Y90 > 0.2 {
		t.Fatalf("despiked metronome baseline must score near zero, got y50=%v y90=%v", out.Y50, out.Y90)
	}

	// Control: same stream with despiking off must pin the score --
	// otherwise this test wouldn't be exercising anything.
	sc2 := NewScorer(testConfig(), testCurve(t))
	var out2 Output
	for i := 0; i < 300; i++ {
		w := baselineWindow()
		if i > 10 && i%11 == 0 {
			w.P50Ns, w.P90Ns = 25_000_000, 40_000_000
		}
		out2 = sc2.Score(w)
	}
	if out2.Y50 < 0.9 {
		t.Fatalf("control (no despike) should pin y50 near 1 on this stream, got %v", out2.Y50)
	}
}

func TestScorerDespikeStillDetectsSustainedContention(t *testing.T) {
	// Despiking must not blind the scorer to real contention: a
	// sustained 3x latency shift (typical aggressor effect) must drive
	// y50 high with despike_k=8.
	cfg := testConfig()
	cfg.DespikeK = 8
	sc := NewScorer(cfg, testCurve(t))
	for i := 0; i < 60; i++ {
	...
	for i := 0; i < 120; i++ {
		w := baselineWindow()
		w.P50Ns, w.P90Ns = 120_000, 180_000 // 3x the baseline
		out = sc.Score(w)
	}
	if out.Y50 < 0.9 {
		t.Fatalf("sustained 3x contention must score high despite despiker, got y50=%v", out.Y50)
	}
}
